...
from app.schemas.wallet import WalletBalanceResponse, WalletTransactionResponse, WalletDepositRequest, WalletWithdrawRequest
...
async def deposit_to_wallet(
    ...
    repo = WalletRepo(db)
    wallet = await repo.get_or_create(current_user.id)
    wallet = await repo.add_balance(wallet, request.amount, request.description or "واریز به کیف پول")
    return WalletBalanceResponse(balance=float(wallet.balance))
...
async def withdraw_from_wallet(
    ...
    wallet = await repo.deduct_balance(wallet, request.amount, request.description or "برداشت از کیف پول")
    return WalletBalanceResponse(balance=float(wallet.balance))
