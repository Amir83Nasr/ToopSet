...
| PATCH  | `/api/v1/auth/profile`  | کاربر لاگین‌شده                | ویرایش نام کامل و/یا رمز عبور جدید بدون نیاز به رمز فعلی.                                                          |
...
| POST   | `/api/v1/bookings`                     | کاربر لاگین‌شده    | ساخت رزرو pending برای یک `slot_id`. رزرو نبودن سانس و `version` سانس بررسی می‌شود و مهلت پرداخت ۱۰ دقیقه‌ای ثبت می‌شود.      |
